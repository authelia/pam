...
APT_CACHE_ROOT="/buildkite/.cache/apt"
APT_DISTS=("stable")
APT_ARCHS=("binary-amd64" "binary-arm64" "binary-armhf")

echo "--- :debian: :fedora: :ubuntu: Updating APT repository for release: pam_autheia ${BUILDKITE_TAG}"
git clone git@github.com:authelia/apt.git --single-branch --depth 1

mkdir -p "${APT_DB_ROOT}" "${APT_REPO_ROOT}/pool/main/dists" "${APT_CACHE_ROOT}"
...
for FILE in *.deb; do
  echo "/pool/main/dists/stable/${FILE} https://github.com/authelia/pam/releases/download/${BUILDKITE_TAG}/${FILE} 301" >> "${APT_REPO_ROOT}/_redirects"
  mv "${FILE}" "${APT_CACHE_ROOT}/"
done

cp -a "${APT_CACHE_ROOT}" "${APT_REPO_ROOT}/pool/main/dists/stable"
...
GIT_COMMITTER_NAME="autheliabot" GIT_COMMITTER_EMAIL="autheliabot@users.noreply.github.com" \
  git -c user.signingkey=security@authelia.com commit -S -m "release: pam_authelia ${BUILDKITE_TAG}"
